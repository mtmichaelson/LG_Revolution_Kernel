...
/*MOD 0019235:  [LTE] Monitoring code & LTE Crash Dump Handling */
#ifdef LTE_CRASH_DUMP
extern void lte_sdio_rx(void *data);
void hw_wdt_rst(void* data);
#endif /* LTE_CRASH_DUMP */
/*END: 0019235 seongmook.yim@lge.com 2011-07-07 */
...
/* ADD 0015440: [LTE] LTE image (singed_app.bin) partition is changed to system from persist */ 
  ...
  #if defined(ENG) || defined(USERDEBUG)
    flag_lte_parition   =1;
...
/* BEGIN 0016693: daeok.kim@lge.com 20110222 */
/* ADD 0016693: [LTE] Update for OSLT (open source): #else condition is added  */
  #elif defined(USER)
    flag_lte_parition =0;
    static const char LTE_CPU_persist[]		= "/persist/lte_images/signed_app.bin";
  	static const char LTE_CPU[]		= "/etc/lte_images/signed_app.bin";
    ...
    static const char LTE_CPU_persist[]		= "/persist/lte_images/signed_app.bin";
  	static const char LTE_CPU[]		= "/etc/lte_images/signed_app.bin";
    LBOOT_ERROR("[LTE_IMAGE] ETC, system \n");
  #endif
/* END 0016693: daeok.kim@lge.com 20110222 */   
...
/*MOD 0019235:  [LTE] Monitoring code & LTE Crash Dump Handling */
#ifdef LTE_CRASH_DUMP	
gLte_sdio_info->lte_crash_dump_flag = FALSE;
gLte_sdio_info->error_packet = FALSE;
#endif /* LTE_CRASH_DUMP */
/*END: 0019235 seongmook.yim@lge.com 2011-07-07 */
	int error = 0, i =0;
	unsigned char       lte_gpr_value = 0;
...
/*BEGIN: 0019235 seongmook.yim@lge.com 2011-07-07 */
/*MOD 0019235:  [LTE] Monitoring code & LTE Crash Dump Handling */
/* Working a hw_wdt_rst_work */
#ifdef LTE_CRASH_DUMP
	//printk("[LTE_HW_WDT] Working a hw_wdt_rst_work\n");
	queue_work(gLte_sdio_info->hw_wdt_rst_workq, &gLte_sdio_info->hw_wdt_rst_worker);
...
#ifdef SDCC_STATUS_MONITORING
	printk("[LTE_HW_WDT] GPIO_LTE_HW_WDT_RST (GPIO_2)= %d \n",gpio_get_value(2));
	printk("[LTE_HW_WDT] GPIO_LTE_SDIO_PWR (GPIO_154)= %d \n",gpio_get_value(154));
	printk("[LTE_HW_WDT] GPIO_LTE_SDIO_RESET  (GPIO_157)= %d \n",gpio_get_value(157));
	printk("[LTE_HW_WDT] GPIO_L2K_LTE_WAKEUP  (GPIO_153)= %d \n",gpio_get_value(153));
	printk("[LTE_HW_WDT] GPIO_L2K_LTE_STATUS (GPIO_32)= %d \n",gpio_get_value(32));
	printk("[LTE_HW_WDT] GPIO_L2K_HOST_WAKEUP (GPIO_18)= %d \n",gpio_get_value(18));
	printk("[LTE_HW_WDT] GPIO_L2K_HOST_STATUS (GPIO_123)= %d \n",gpio_get_value(123));
...
/*BEGIN: 0019235 seongmook.yim@lge.com 2011-07-07 */
/*MOD 0019235:  [LTE] Monitoring code & LTE Crash Dump Handling */
/* Init a workqueue for HW_WDT_RST */
	#ifdef LTE_CRASH_DUMP
	//printk("[LTE_HW_WDT] Init a workqueue for HW_WDT_RST\n");
	INIT_WORK(&gLte_sdio_info->hw_wdt_rst_worker, hw_wdt_rst );
	gLte_sdio_info->hw_wdt_rst_workq = create_workqueue("hw_wdt_rst");
	if (NULL == gLte_sdio_info->hw_wdt_rst_workq)
	{
		printk("[LTE_HW_WDT] Failed to create work queue\n");
		return -ENOMEM;
	}
	#endif /* LTE_CRASH_DUMP */
/*END: 0019235 seongmook.yim@lge.com 2011-07-07 */
...
void hw_wdt_rst(void* data)
{
	...
	printk("\n[LTE_HW_WDT] GPIO_LTE_HW_WDT_RST (GPIO_2)= %d \n",gpio_get_value(2));
	printk("[LTE_HW_WDT] GPIO_LTE_SDIO_PWR (GPIO_154)= %d \n",gpio_get_value(154));
	printk("[LTE_HW_WDT] GPIO_LTE_SDIO_RESET  (GPIO_157)= %d \n",gpio_get_value(157));
	printk("[LTE_HW_WDT] GPIO_L2K_LTE_WAKEUP  (GPIO_153)= %d \n",gpio_get_value(153));
	printk("[LTE_HW_WDT] GPIO_L2K_LTE_STATUS (GPIO_32)= %d \n",gpio_get_value(32));
...
/* MOD 0018470 : [LTE] LTE ASSERT CASE report available in SDIO not working */
	if(gpio_get_value(GPIO_L2K_LTE_STATUS) == 1)
	{				  
	/* START: 0019465 seongmook.yim@lge.com 2011-08-28 */
	/* ADD 0019465 : [LTE] SDIO TX blocking in HW WDT RST */
		printk("[LTE_HW_WDT] SDIO tx blocking in lte_boot.c\n");
		gLte_sdio_info->flag_tx_blocking = TRUE;
	/* END: 0019465 seongmook.yim@lge.com 2011-08-28 */
		error_log_lte_hw_wdt = kzalloc(LTE_WDT_LOG_SIZE, GFP_KERNEL);
		error_log_lte_hw_wdt = "LTE assert! But receiving asset log has failed!\n";
		...
		/* lte_crash_dump_flag is set & use in a parsing process in lte_him.c */
		gLte_sdio_info->lte_crash_dump_flag = TRUE;
		//printk("[LTE_HW_WDT] lte_crash_dump flag is set!! = %d\n",gLte_sdio_info->lte_crash_dump_flag);
		
		wake_lock(&gLte_sdio_info->rx_wake_lock);

		while (1)
		{
		/* Read SDIO for Error_Packet */
		lte_sdio_rx(temp);

		/* SDIO read in every 1sec */
		if(gLte_sdio_info->error_packet == TRUE)
		{
			printk("[LTE_ASSERT] Error Packet find success!\n");
			break;
		}

		if(hw_wdt_rst_count > 1)
		{
			printk("[LTE_ASSERT] hw_wdt_rst_count is %d \n",hw_wdt_rst_count);
			break;
		}

		hw_wdt_rst_count++;
		}
		/* Reset hw_wdt_rst_count */
		hw_wdt_rst_count = 0;		
	}
