...
void msmsdcc_print_sdcc_status();
...
/*MOD 0019235:  [LTE] Monitoring code & LTE Crash Dump Handling */
#ifdef SDCC_STATUS_MONITORING
	memset( &gLte_sdcc_status, 0xFF, sizeof(gLte_sdcc_status));
#endif
/*END: 0019235 seungyeol.seo@lge.com 2011-07-07 */
	return 0;
}
...
/*MOD 0019235:  [LTE] Monitoring code & LTE Crash Dump Handling */
#ifdef SDCC_STATUS_MONITORING
	del_timer(&gLte_sdio_info->sdio_block_ckeck_1); 
	del_timer(&gLte_sdio_info->sdio_block_ckeck_2); 
	del_timer(&gLte_sdio_info->sdio_block_ckeck_3); 
#endif /* SDCC_STATUS_MONITORING */
/*END: 0019235 seungyeol.seo@lge.com 2011-07-07 */
...
/*MOD 0019235:  [LTE] Monitoring code & LTE Crash Dump Handling */
#ifdef SDCC_STATUS_MONITORING
void sdio_block_ckeck_1_timer_func(unsigned long arg)
{
    int* abc;

	LTE_ERROR("Mutex Locked!!\n");
	msmsdcc_print_sdcc_status();
    abc = NULL;
    *abc= 0;	
}

void sdio_block_ckeck_2_timer_func(unsigned long arg)
{
    int* abc;

	LTE_ERROR("SDIO Claim Fails!!\n");
...
void sdio_block_ckeck_3_timer_func(unsigned long arg)
{
    int* abc;

	LTE_ERROR("Read Done Checking Fails!!\n");
...
{
	init_timer(&gLte_sdio_info->sdio_block_ckeck_1);
	gLte_sdio_info->sdio_block_ckeck_1.expires = jiffies + 5*HZ;
	gLte_sdio_info->sdio_block_ckeck_1.data = 0;
	gLte_sdio_info->sdio_block_ckeck_1.function = sdio_block_ckeck_1_timer_func;
	add_timer(&gLte_sdio_info->sdio_block_ckeck_1);
}

int sdio_block_ckeck_2_timer_init(void)
{
	init_timer(&gLte_sdio_info->sdio_block_ckeck_2);
	gLte_sdio_info->sdio_block_ckeck_2.expires = jiffies + 10*HZ;
	gLte_sdio_info->sdio_block_ckeck_2.data = 0;
	gLte_sdio_info->sdio_block_ckeck_2.function = sdio_block_ckeck_2_timer_func;
	add_timer(&gLte_sdio_info->sdio_block_ckeck_2);
}
int sdio_block_ckeck_3_timer_init(void)
{
	init_timer(&gLte_sdio_info->sdio_block_ckeck_3);
	gLte_sdio_info->sdio_block_ckeck_3.expires = jiffies + 10*HZ;
	gLte_sdio_info->sdio_block_ckeck_3.data = 0;
	gLte_sdio_info->sdio_block_ckeck_3.function = sdio_block_ckeck_3_timer_func;
	add_timer(&gLte_sdio_info->sdio_block_ckeck_3);
}
...
//	int pr_i;
	unsigned char data;
	void __iomem *base = 0xe0872000 + 0x034;

	unsigned char print_buff[100];

	int print_index=0;
	for( i = 0 ;  i < 30 ; i++)
	{
		if(i==29 || gLte_sdcc_status.tracking[i] == enum_msmsdcc_max){
			print_buff[print_index-1]='\0';
			break;
		}

		if(gLte_sdcc_status.tracking[i] >= 10){
			print_buff[print_index] = gLte_sdcc_status.tracking[i]/10 + '0';
			print_buff[print_index+1] = gLte_sdcc_status.tracking[i]%10 + '0';
			print_buff[print_index+2] = '-';
			print_index += 3;
		}
		else{
			print_buff[print_index] = gLte_sdcc_status.tracking[i] + '0';
			print_buff[print_index+1] = '-';
			print_index += 2;
		}
	}
	...
	printk("(%s)",print_buff);
	//printk("MCI(0x%x) ",readl(base));//because of this statement phone is entering into emergency download mode. need to check later Nagi
...
/*
	printk("CCCR is ");
	for(i=0x00 ; i<0x14 ; i++){
		mmc_io_rw_direct_customized(gLte_sdio_info->func->card, 0, 0, i, 0, &data);
		printk("addr(0x%x) 0x%x   ",i, data);
	}


	printk("\nFBR is ");
	for(i=0x100 ; i<0x111 ; i++){
		mmc_io_rw_direct_customized(gLte_sdio_info->func->card, 0, 0, i, 0, &data);
		printk(" addr(0x%x) 0x%x ", i, data);
	}
	printk("\n");
