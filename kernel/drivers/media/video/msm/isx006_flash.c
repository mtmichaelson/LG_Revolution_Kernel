...
#define ISX006_AGC_THRESHOLD		  0x06CC  
...
/*---------------------------------------------------------------------------
  isx006_flash_led_mode_auto
   ---------------------------------------------------------------------------*/
int32_t isx006_flash_led_mode_auto(int8_t flash_state)
{
	struct isx006_flash_t *ctrl_flash = &isx006_ctrl_flash;
	uint16_t reg_val = 0;
	int32_t  rc = 0,i;
	...
	case FLASH_STATE_PREFLASH:
		 ...
		 if(reg_val > ISX006_AGC_THRESHOLD){	
...
/*---------------------------------------------------------------------------
  isx006_update_flash_state
   ---------------------------------------------------------------------------*/
int32_t isx006_update_flash_state(int8_t flash_state)
{
	struct isx006_flash_t *ctrl_flash = &isx006_ctrl_flash;
	int32_t rc = 0;
	...
	if(ctrl_flash->test_mode_enable){
		...
		if(ctrl_flash->led_mode == LED_MODE_AUTO)
			isx006_flash_led_mode_auto(flash_state);
