...
static int isx006_set_fps_range(int fps)
{
	...
	if(!atomic_read(&pll_mode)== CAMERA_TYPE_MAIN_FOR_VT) // snapshot mode
	{
...
int32_t isx006_set_scene_mode(int8_t mode)
{
	...
	case CAMERA_SCENEMODE_NORMAL:
		if(atomic_read(&pll_mode)== CAMERA_TYPE_MAIN_FOR_VT){				
...
int32_t isx006_sensor_config(void __user *argp)
{
	...
	case CFG_SET_FLASH_STATE:
		rc = isx006_update_flash_state(cfg_data.flash_state);
		break;
	case CFG_SET_FPS:
	  	 if(atomic_read(&pll_mode)== CAMERA_TYPE_MAIN_FOR_VT) 
...
static DEVICE_ATTR(brightness, S_IRUGO|S_IWUSR|S_IWGRP, NULL, isx006_brightness_store);
...
static DEVICE_ATTR(flash_enable, S_IRUGO|S_IWUSR|S_IWGRP, NULL, isx006_flash_enable_store);
...
static DEVICE_ATTR(flash_state, S_IRUGO|S_IWUSR|S_IWGRP, NULL, isx006_flash_state_store); 
...
static ssize_t isx006_mclk_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t size)
{
 ...
 static DEVICE_ATTR(mclk, S_IRWXU|S_IRWXG|S_IROTH|S_IXOTH, isx006_mclk_show, isx006_mclk_store);
 ...
 static ssize_t isx006_init_code_store(struct device* dev, struct device_attribute* attr, const char* buf, size_t n)
{
    ...
    /* 0 : main camera to support the 5M YUV snapshot (preview with 720P, 15fps) 
    	    1 : secondary camera to support the 1.3M YUV snapshot (preview with VGA, 22 fps) 
    	    2 : main camera to support the preview with 720P, 30 fps  
     	    3 : secondary camera to support the preview with VGA, 30 fps
      */
	if(val != CAMERA_TYPE_MAIN && val != CAMERA_TYPE_MAIN_FOR_VT) {
		CAM_ERR("isx006: invalid pll type[%d] \n",val);
		val = CAMERA_TYPE_MAIN;
	}
...
static DEVICE_ATTR(init_code, S_IRUGO|S_IWUSR|S_IWGRP, NULL, isx006_init_code_store);
	                                 ...
	                                 struct msm_sensor_ctrl *s)
{
	...
	s->s_camera_type = BACK_CAMERA_2D;//saravanan
	s->s_mount_angle = 0;//saravanan
