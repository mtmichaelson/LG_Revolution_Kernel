...
void lm3559_enable_flash_mode(int state)
{
	...
	if(state == LM3559_LED_LOW){ 		
		...
		/* 0111 : 450 mA*/	 
		lm3559_i2c_write(lm3559_client,LM3559_REG_FLASH_BRIGHTNESS,0x77);
	}
...
#ifdef CONFIG_MACH_LGE_BRYCE
	check_rev = CheckHWRev();
	if(check_rev == 0x44)/*Rev.D*/
		lm3559_pdata->gpio_flen = FLASH_EN_GPIO_NUMBER_REV_D;
	else
