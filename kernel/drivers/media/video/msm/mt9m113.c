...
/*Saravanan Focus Distance Support MS6910 Merge*/
...
static int mt9m113_set_fps_range(int fps)
{
	...
	if(!atomic_read(&init_reg_mode)== CAMERA_TYPE_MAIN_FOR_VT) // snapshot mode
	{
...
int mt9m113_sensor_config(void __user *argp)
{
	...
	switch (cfg_data.cfgtype) {
	 ...
	 case CFG_SET_BRIGHTNESS:
		 ...
		 break;
	  case CFG_SET_FPS:
	  	 if(atomic_read(&init_reg_mode)== CAMERA_TYPE_SECONDARY_FOR_VT){ /* variable frame rate */
...
int mt9m113_init_sensor_reg(void)
{
	...
	if(atomic_read(&init_reg_mode)== CAMERA_TYPE_SECONDARY_FOR_VT){
...
static DEVICE_ATTR(mclk, S_IRWXU|S_IRWXG|S_IROTH|S_IXOTH, NULL, mt9m113_mclk_store);
...
static DEVICE_ATTR(init_code, S_IRUGO|S_IWUSR|S_IWGRP, NULL, mt9m113_init_code_store);
...
int mt9m113_sensor_release(void)
{
    ...
    atomic_set(&init_reg_mode,CAMERA_TYPE_SECONDARY);
	                                 	...
	                                 	struct msm_sensor_ctrl *s)
{
	...
	s->s_camera_type = FRONT_CAMERA_2D;//Dewesh_260711
	s->s_mount_angle =  180;
