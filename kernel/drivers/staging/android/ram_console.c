...
/* kwangdo.yi imsi for test 10.10.26 S 
...
/* BEGIN: 0014783 hyogook.lee@lge.com 2011-01-25 */
/* ADD: 0014783: [Booting] Chargerlogo Enter Condition Change Merged*/
// this is needed to prevent going into the charging mode
// LGE_CHANGE_S [dojip.kim@lge.com] 2010-08-04, clean the ram console when normal shutdown
#if 1//defined(CONFIG_LGE_RAM_CONSOLE_CLEAN)
...
/* END: 0014783 hyogook.lee@lge.com 2010-01-25 */
...
static int __init ram_console_late_init(void)
{
	...
	char dload_reset_buf[50];
#endif

/* BEGIN: 0015730 jihoon.lee@lge.com 20110209 */
/* MOD 0015730: [POWER_OFF] Change smem init sequence and smem index for dload boot up */
	/*
	 * smem_sreset_hwreset
	 * smem_sreset_flag
	 * smem_hw_reset_flag
	...
	*/
	...
	printk(KERN_ERR "ram_console: sreset_hwreset %x\n", smem_sreset_hwreset);
/* END: 0015730 jihoon.lee@lge.com 20110209 */
	if(smem_sreset_hwreset != NULL)
	{
		printk(KERN_ERR "ram_console: sreset_hwreset Val %x\n", *smem_sreset_hwreset);
		if(*smem_sreset_hwreset == 0xDDDEADDD)
		{
			entry = create_proc_entry("last_kmsg_arm9", S_IFREG | S_IRWXUGO, NULL);
			if (!entry) {
		...
		else if((*smem_sreset_hwreset == 0xDDDDDEAD)&&(ram_console_old_log == NULL))
		{
			memset(dload_reset_buf, 0, sizeof(dload_reset_buf));
			sprintf(dload_reset_buf, "\n%s\n", "dload reset detected!!");
			ram_console_old_log = kmalloc(sizeof(dload_reset_buf), GFP_KERNEL);
			...
			else
			{
				...
				//1 create last_kmsg to notify this is just after downloading
