...
int overlay_set=0, overlay_unset=0;
...
/* CONFIG_MACH_LGE_BRYCE  chanhee.park@lge.com   2010.11.18
      for skype VT
*/
#ifdef CONFIG_MACH_LGE_BRYCE  
		if((z_order >= 0) && (z_order <= 2)){
			 return 0;
		}
#endif
...
/* CONFIG_MACH_LGE_BRYCE   	chanhee.park@lge.com   
		to control the z_order of VG2 pipe for skype VT
*/
#ifdef CONFIG_MACH_LGE_BRYCE
		if(pipe->pipe_num >= OVERLAY_PIPE_VG1){
			struct mdp4_overlay_pipe *pipe_test = NULL;
			uint32_t   z_order_pipe = 0;

			printk("%s: pipe_num:%d  request z-order:%d\n",__func__,pipe->pipe_num,z_order_vg_layer);
			if(z_order_vg_layer == 1)
				req->z_order = 1;
			else
				req->z_order = 0;	
				
			if(pipe->pipe_num == OVERLAY_PIPE_VG1){
				printk("%s:pipe->pipe_num == OVERLAY_PIPE_VG1\n",__func__);
				pipe_test = &ctrl->plist[OVERLAY_PIPE_VG2+2];
				
			}
			else if(pipe->pipe_num == OVERLAY_PIPE_VG2){
				printk("%s:pipe->pipe_num == OVERLAY_PIPE_VG2\n",__func__);
				pipe_test = &ctrl->plist[OVERLAY_PIPE_VG1+2];
			}

			if(pipe_test){
				if(pipe_test->pipe_used){
				  	z_order_pipe = pipe_test->mixer_stage - MDP4_MIXER_STAGE0;
					if(z_order_pipe == 1)
						req->z_order = 0;
					else
						req->z_order = 1;
...
/* Start : QCT case number 00442697 : to change z-order  - chanhee.park@lge.com */
...
#if 1 /* QCT case number 00442697 : to change z-order  - chanhee.park@lge.com */
...
int mdp4_overlay_pipe_used(int index){

	struct mdp4_overlay_pipe *pipe;
	pipe = &ctrl->plist[index-1];
	//printk("%s #%d pipe is %s\n",__func__,index,pipe->pipe_used?"used":"not used");
	return pipe->pipe_used;
}

/* CONFIG_MACH_LGE_BRYCE   chanhee.park@lge.com 
		to set z-order when creating VG layer 
*/ 
#ifdef CONFIG_MACH_LGE_BRYCE 
int mdp4_overlay_set_zorder(int zorder){
...
EXPORT_SYMBOL(mdp4_overlay_pipe_used);
