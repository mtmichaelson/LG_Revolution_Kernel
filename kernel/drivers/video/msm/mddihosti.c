...
/* jihye.ahn@lge.com   2010-11-20    for switching mddi type 1 & type 2 */
#ifndef FEATURE_MDDI_DISABLE_REVERSE
...
static void mddi_report_state_change(uint32 int_reg)
{
	...
	if (int_reg & MDDI_INT_IN_HIBERNATION) {
		...
		if (mddi_client_type == 2) {
			/* LGE_S jihye.ahn@lge.com 2010-11-08 to fix LCD refresh problems on MDDI 2 */
			#if 1
			mddi_host_reg_out(PAD_CTL, 0x403f850f);
			mddi_host_reg_out(PAD_CAL, 0x1e23bc20);
			#else /* QCT original */
			mddi_host_reg_out(PAD_CTL, 0x402a850f);
			mddi_host_reg_out(PAD_CAL, 0x10220020);
			#endif
			/* LGE_E jihye.ahn@lge.com 2010-11-08 to fix LCD refresh problems on MDDI 2 */
...
{
	uint32 pad_reg_val;
	uint32 pad_io_ctl;    //jihye.ahn@lge.com
	mddi_host_cntl_type *pmhctl = &(mhctl[host_idx]);
...
/*
	sw_ct : Vsw1 (mv) - 301-> 430 mv
	Vsw2 (mv) - 226 -> 323 mv
	Vsw3 (mv) - 151-> 215 mv
	emph_str : 30 -> 50%
*/
	pad_reg_val = 0x4689d420; //0x40884020; // increase of emph_str to improve mddi2 E0 signal
#endif
...
/* jihye.ahn 2010-11-20 - MDDI PAD CTL REG Setting */
#ifdef FEATURE_MDDI_PAD_CTRL
        /* jihye.ahn@lge.com 2011-11-26  for mddi stability */
        mddi_host_reg_out(DRIVER_START_CNT, 0xC00060); //600060
