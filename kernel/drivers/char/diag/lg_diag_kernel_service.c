...
/* BEGIN: 0014568 jihoon.lee@lge.com 20110122 */
/* MOD 0014568: [TESTMODE] diagchar_write exception handling */
...
/* END: 0014568 jihoon.lee@lge.com 2011022 */
...
/* END: 0014568 jihoon.lee@lge.com 2011022 */
		...
		if(!driver->write_ptr_svc) {
			printk(KERN_ERR "%s, driver->usb_write_ptr_svc allocation failed\n", __func__);
...
*/
	...
	if ((unsigned int) enc.dest >=
		...
		if (err) {
#if 1//def LG_DIAG_DEBUG		 
		printk(KERN_ERR "LG_FW : usb_diag_write error, overflow write failed\n");
#endif
			/*Free the buffer right away if write failed */
			diagmem_free(driver, buf_hdlc, POOL_TYPE_HDLC);
			ret = -EIO;
	...
	if (pkt_type == DATA_TYPE_RESPONSE) {
		...
		if (err) {
#if 1//def LG_DIAG_DEBUG		 
			printk(KERN_ERR "LG_FW : usb_diag_write error, last packet write failed\n");
#endif
...
// MOD  0010599: [ETA/MTC] MTC capture ¹× touch ±â´É
//#undef LG_DIAG_DEBUG
// END: 0010599 alan.park@lge.com 2010.11.07   
void diagpkt_commit (PACK(void *)pkt)
{
/* BEGIN: 0014165 jihoon.lee@lge.com 20110116 */
/* MOD 0014165: [KERNEL] diag_commit exception handling */
  unsigned int length = 0;
  unsigned char *temp = NULL;
  int type = DIAG_DATA_TYPE_RESPONSE;
/* END: 0014165 jihoon.lee@lge.com 20110116 */
#ifdef LG_DIAG_DEBUG
  int i;
#endif
...
/* LGE_CHANGE_S [jihoon.lee@lge.com] 2010-02-22, LG_FW_MTC */
// enable to send more than maximum packet size limitation
#if 1 // def LG_FW_MTC //defined (LG_FW_DIAGTEST) && defined (LG_FW_MTC)
void diagpkt_commit_mtc(PACK(void *)pkt)
{
#ifdef LG_DIAG_DEBUG
  int i;
