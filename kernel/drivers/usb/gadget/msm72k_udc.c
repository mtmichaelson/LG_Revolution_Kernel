...
/* [yk.kim@lge.com] 2011-03-22, ums workaround patch */
...
#ifdef CONFIG_MACH_LGE_BRYCE
void msm_hsusb_set_state(enum usb_device_state state)
#else
...
#ifdef CONFIG_MACH_LGE_BRYCE
enum usb_device_state msm_hsusb_get_state(void)
#else
...
/* BEGIN:0011642 [yk.kim@lge.com] 2010-11-30, usb hub reset func */
#ifdef CONFIG_MACH_LGE_BRYCE
static void hub_reset(void)
{
	int rc;
	uint32_t usb_hub_gpio_cfg_value = GPIO_CFG(35, 0, GPIO_CFG_OUTPUT, GPIO_CFG_PULL_UP, GPIO_CFG_2MA);

	rc = gpio_tlmm_config(usb_hub_gpio_cfg_value, GPIO_CFG_ENABLE);
	USB_DBG( "gpio_tlmm_config(%#x)=%d\n", usb_hub_gpio_cfg_value, rc);
...
#endif
/* END:0011642 [yk.kim@lge.com] 2010-11-30 */
#ifdef CONFIG_LGE_USB_GADGET_SUPPORT_FACTORY_USB //ulpi_read from SBA 2030_07
...
#ifdef CONFIG_MACH_LGE_BRYCE
	pr_info("\n=+=+=+=+=+=+=usb_chg_detect chg_type=[%d] max pwr = [%d]\n", temp, maxpower);
#endif
...
// BEGIN: 0013016: hyunjong.do@lge.com 2010-12-24
// DEL: 0013016: not entering sleep mode when TA inserted 
//		wake_unlock(&ui->wlock);
// END: 0013016: hyunjong.do@lge.com 2010-12-24
	}
}
...
#if 0
		/* [yk.kim@lge.com] 2011-03-22, ums workaround patch */
		if (is_b_sess_vld() && (android_get_product_id()==lg_ums_pid))
		{
			printk ("LG FW :: UMS return IRQ_HANDLED\n");
			return IRQ_HANDLED;
		}
#else
		printk ("LG FW :: UMS Workaround removed");
#endif
		spin_lock_irqsave(&ui->lock, flags);
...
static void usb_reset(struct usb_info *ui)
{
...
#ifdef CONFIG_LGE_USB_GADGET_SUPPORT_FACTORY_USB
	if(udc_cable==LG_NONE_CABLE && lg_manual_test_mode && !user_cable)
	{
	...
	if(udc_cable==LG_FACTORY_CABLE_56K_TYPE || udc_cable==LG_FACTORY_CABLE_130K_TYPE || udc_cable==LG_FACTORY_CABLE_910K_TYPE)
	{
		...
		if(product_id != lg_factory_pid && udc_cable==LG_FACTORY_CABLE_56K_TYPE)
		{
			spin_lock_irqsave(&ui->lock, flags);
...
#endif
				/* [yk.kim@lge.com] 2011-02-21, charge_only_mode -> unplug */
#endif
				ui->state = USB_STATE_OFFLINE;
				usb_do_work_check_vbus(ui);
