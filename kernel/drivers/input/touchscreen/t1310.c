...
//[Lavanya] disabled logs for touchscreen slow response [start]
//#define TOUCHEY_DEBUG_PRINT	(1)
//[Lavanya] disabled logs for touchscreen response [end]
...
//	s8 ret;
	...
	if (ret != 0) {
		TOUCHE("E: caddr(0x%x),addr(%u),ret(%d)\n", touch_pdev->client->addr, addr, ret);
...
{
//[Lavanya] disabled logs for touchscreen response [start]
#if 0
	int state;

	state = gpio_get_value(GPIO_TOUCH_ATTN);
	TOUCHD("touch_irq_handler PIN STATE: %d t1310_state = %d\n", state,t1310_state);
#endif
//[Lavanya] disabled logs for touchscreen response [end]
...
static ssize_t
touch_count_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	int ret;

	ret = touch_i2c_read(0x33);
	printk("[0x33] 0x%x\n", ret);
...
static ssize_t
touch_count_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	int ret;
	u8 data;
	...
	ret = touch_i2c_write(0x33, data);
	ret = touch_i2c_read(0x33);
	printk("[0x33] input:0x%x -> output:0x%x\n", data, ret);
...
touch_fd_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	t1310_state = FIRMWARE_DOWNLOAD;
	touch_power(0);
	touch_power(1);
	//mdelay(400);
	FirmwareUpgrade_main(10);
...
static struct device_attribute touch_device_attrs[] = {
	...
	__ATTR(count, S_IRUGO | S_IWUSR, touch_count_show, touch_count_store),
...
/*Auto-Firmware Upgrade*/
	//mdelay(400);
	ret = touch_i2c_read(0x9d);
	printk("Touch 0x9d: 0x%x\n", ret);
	if((ret > 0 && ret != 0x0b) || ret == 0){
		t1310_state = FIRMWARE_DOWNLOAD;
		FirmwareUpgrade_main(11);
		t1310_state = NOT_BOOTING;
...
#ifndef CONFIG_HAS_EARLYSUSPEND	//lavanya commented as touchscreen is not using Normal resume and suspend as intimated from base engineer
#ifdef CONFIG_PM
