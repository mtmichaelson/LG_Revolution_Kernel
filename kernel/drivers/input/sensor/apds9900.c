...
#define PROX_INT_HIGH_TH	(120) /* near interrupt threshold */
#define PROX_INT_LOW_TH	(100) /* far interrupt threshold */
/*
 * Structs
 */
...
static void apds9900_change_ps_threshold(struct i2c_client *client)
{
	...
	data->ps_data =	i2c_smbus_read_word_data(client, CMD_WORD|APDS9900_PDATAL_REG);
		...
		i2c_smbus_write_word_data(client, CMD_WORD|APDS9900_PILTL_REG, data->pilt);
		i2c_smbus_write_word_data(client, CMD_WORD|APDS9900_PIHTL_REG, 0);
		...
		i2c_smbus_write_word_data(client, CMD_WORD|APDS9900_PIHTL_REG, data->piht);
...
static void apds9900_change_als_threshold(struct i2c_client *client)
{
	struct apds9900_data *data = i2c_get_clientdata(client);
        ...
        data->als_data =(unsigned int)((iac*69)/100);
...
/* jihye.ahn 2011-02-01	 fix to be not going into PS ISR */
#ifdef CONFIG_MACH_LGE_BRYCE
...
/* jihye.ahn@lge.com 2011-04-23 
 * restore the interface for sensor on/off.
 */
         apds9900_set_enable(client, val);


	return count;
}

static DEVICE_ATTR(enable, S_IWUSR | S_IRUGO|S_IWGRP |S_IRGRP |S_IROTH,
...
static DEVICE_ATTR(atime,  S_IWUSR | S_IRUGO|S_IWGRP |S_IRGRP |S_IROTH,
...
static DEVICE_ATTR(ptime,  S_IWUSR | S_IRUGO|S_IWGRP |S_IRGRP |S_IROTH,
...
static DEVICE_ATTR(wtime,  S_IWUSR | S_IRUGO|S_IWGRP |S_IRGRP |S_IROTH,
...
static DEVICE_ATTR(ailt,  S_IWUSR | S_IRUGO|S_IWGRP |S_IRGRP |S_IROTH,
...
static DEVICE_ATTR(aiht,  S_IWUSR | S_IRUGO|S_IWGRP |S_IRGRP |S_IROTH,
...
static DEVICE_ATTR(pilt,  S_IWUSR | S_IRUGO|S_IWGRP |S_IRGRP |S_IROTH,
...
static DEVICE_ATTR(piht,  S_IWUSR | S_IRUGO|S_IWGRP |S_IRGRP |S_IROTH,
...
static DEVICE_ATTR(pers,  S_IWUSR | S_IRUGO|S_IWGRP |S_IRGRP |S_IROTH,
...
static DEVICE_ATTR(config,  S_IWUSR | S_IRUGO|S_IWGRP |S_IRGRP |S_IROTH,
...
static DEVICE_ATTR(ppcount,  S_IWUSR | S_IRUGO|S_IWGRP |S_IRGRP |S_IROTH,
...
static DEVICE_ATTR(control,  S_IWUSR | S_IRUGO|S_IWGRP |S_IRGRP |S_IROTH,
...
static DEVICE_ATTR(enable1, S_IWUSR | S_IRUGO|S_IWGRP |S_IRGRP |S_IROTH,
...
static int apds9900_device_init(struct i2c_client *client)
{
	...
	apds9900_set_piht(client, PROX_INT_HIGH_TH); 
        ...
        apds9900_set_enable(client, 0x3F);

        data->pilt = PROX_INT_LOW_TH;
        data->piht = PROX_INT_HIGH_TH;
...
/* kwangdo.yi@lge.com  E */	
	...
	*/
    ...
    /*jihye.ahn@lge.com 2011-04-23 
     * to add delay time before the sensor init.
     */
            mdelay(10);
	...
	*/
	...
	/* jihye.ahn  2011-02-07 fix irq warning for prox sensor */
         set_irq_wake(papds9900_data->irq, 1);
...
/*BEGIN: 140mA Idle Current bug Fix Kiran.Kanneganti@lge.com*/	
	.suspend = NULL,
	.resume	= NULL,
/*END: 140mA Idle Current bug Fix Kiran.Kanneganti@lge.com*/	
