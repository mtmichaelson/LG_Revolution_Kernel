...
/* kwangdo.yi@lge.com 10.10.08 E */

/* kwangdo.yi@lge.com 10.11.25 S 
 * 0011163: [kernel] ERS message display
 */
extern char *ram_misc_buffer;
#endif
#endif
/* kwangdo.yi2lge.com 10.11.29 E */
 ...
 */
	...
	if(lge_error_analyzing == 1){
		char* temp;
		sprintf(crash_buf[lge_error_fb_cnt],"[<%08lx>] ",  where);
...
#endif
#endif	/* kwangdo.yi@lge.com 10.11.29 E *//* kwangdo.yi@lge.com 10.10.26 E */	
	{ /* original */
...
#else
	static int die_counter, dump_type;
	char *temp;
#endif
	int ret;
 ...
 */
	 ...
	 * 0012869 : add the hidden reset */
#if defined (CONFIG_LGE_HIDDEN_RESET_PATCH)
	atomic_notifier_call_chain(&panic_notifier_list, 0, "[MC] traps\n");
#endif
	...
	*ram_misc_buffer++=0x50;
	*ram_misc_buffer++=0x41;
	*ram_misc_buffer++=0x4E;
	*ram_misc_buffer++=0x49;
	*ram_misc_buffer=0x43;
	/* kwangdo.yi@lge.com E */

	/*kwangdo.yi@lge.com 10.11.08 S 
	 * 0010641: [kernel] fix dog task is starved erro
	*/
	msm_proc_comm(PCOM_OEM_AP_PANIC_CMD, 0, 0);
	/*kwangdo.yi@lge.com 10.11.08 E */
	...
	*/
	...
	/* reboot only */
	ret = SMSM_SYSTEM_REBOOT;
	/* kwangdo.yi@lge.com 10.11.05 S 
	 * 0010548: fix bug of going download mode by default 
	 */
	if(ret ==SMSM_SYSTEM_REBOOT)
		dump_type = 1;  
	else if(ret ==SMSM_SYSTEM_DOWNLOAD)
		dump_type = 0;
	else
		dump_type = 2; 
	msm_proc_comm(PCOM_OEM_SET_DUMP_MODE_CMD, &dump_type, 0); 
	/* kwangdo.yi@lge.com 10.11.05 E */
