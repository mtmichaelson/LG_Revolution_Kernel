...
int msm_hsusb_send_serial_number(char *serial_number)
{
	...
	struct hsusb_phy_start_req {
		struct rpc_request_hdr hdr;
		uint32_t length;
/* BEGIN: [yk.kim@lge.com] 2010-12-17, send serial */
#ifdef CONFIG_LGE_USB_GADGET_SUPPORT_FACTORY_USB
		char serial_num[15];
#else
		char serial_num[20];
#endif
/* END: [yk.kim@lge.com] 2010-12-17 */
	} req;
...
int usb_diag_update_pid_and_serial_num(uint32_t pid, const char *snum)
{
	...
	if (ret)
		return ret;
/* LGE_CHANGE_S [jaeho.cho@lge.com] 2011-04-30, unexpected return vaule makes the target freeze */
