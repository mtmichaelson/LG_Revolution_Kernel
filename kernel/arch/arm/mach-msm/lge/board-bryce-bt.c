...
static int bryce_bluetooth_toggle_radio(void *data, bool state)
{
	int ret;
	int (*power_control)(int enable);
	printk(KERN_ERR "b-b-b.c %s\n", __func__);
...
static int bryce_bluetooth_power(int on)
{
	int pin, rc;
	...
	if (on) {
		for (pin = 0; pin < ARRAY_SIZE(bt_config_power_on); pin++) {
			...
			if (rc) {
				printk(KERN_ERR
				       "%s: Pin %d is registered gpio_tlmm_config)=%d rc=%d\n",
				       __func__, pin, bt_config_power_on[pin], rc);
				return -EIO;
			}
			else {
			  printk(KERN_ERR
...
static struct bluetooth_platform_data bryce_bluetooth_data = {
	.bluetooth_power = bryce_bluetooth_power,
	.bluetooth_toggle_radio = bryce_bluetooth_toggle_radio,
};

static struct platform_device msm_bt_power_device = {
	.name = "bt_power",
	.dev = {
		.platform_data = &bryce_bluetooth_data,
	},		
};
...
static struct bluesleep_platform_data bryce_bluesleep_data = {
...
static struct platform_device msm_bluesleep_device = {
	...
	.dev = {
		.platform_data = &bryce_bluesleep_data,
	},	
};
