...
//Suresh: START TestScript Mode check
#include <linux/fs.h>
...
//Suresh: START TestScript Mode check
void put_device_in_test_script_mode(const char *buf);
//Suresh: END
/* ==========================================================================
===========================================================================*/
...
/* 2011.4.23 jaeho.cho@lge.com for usb driver testmode */
	...
	if (pReq->change_usb_driver == CHANGE_MODEM)
		android_set_pid("6200", NULL);
	else if (pReq->change_usb_driver == CHANGE_MASS)
		android_set_pid("6203", NULL);
	else
		pRsp->ret_stat_code = TEST_NOT_SUPPORTED_S;
	return pRsp;
}
        ...
        DIAG_TEST_MODE_F_rsp_type	*pRsp)
{
	...
	if (diagpdev != NULL)
	{
		...
		{			
//BEGIN 0018790 sinyoung.jun@lge.com 2011-04-08
//MOD: 0018790: [BRYCE][BT] fixed to BT testmode fail. 
			msleep(8500); //6sec timeout
//END 0018790 sinyoung.jun@lge.com 2011-04-08
		}
		/*Test Mode Check*/
		else if(pReq->bt==2) 
		{
...
/* BEGIN: 0016870 hyogook.lee@lge.com 2011-02-24 */			
/* DEL 0016870:[DV] TestMode -Sleep Flight Mode Command Restore */
/* 부팅 후 바로 Command 실행하였으나 Factory Test Sequence 변경으로 backlight off 상태에서 실행되므로 원복함. */
		//LGF_SendKey(KEY_END);	
		//msleep(100); //100ms wait
/* BEGIN: 0016870 hyogook.lee@lge.com 2011-02-24 */	
...
//Disable this like MS910 & return sucess. kiran.kanneganti@lge.com
#if 0 //For Checking sleep current in flight mode no need to reboot.
...
// END 0012299: eundeok.bae@lge.com 2010-12-13
		...
		break;
// END 0011665: eundeok.bae@lge.com FTM MODE FOR ONLY KERNEL BOOTING 		
	case AIR_PLAIN_MODE_OFF:	//8.4 cfg plane mode off support. kiran.kanneganti@lge.com
// BEGIN 0011665: eundeok.bae@lge.com FTM MODE FOR ONLY KERNEL BOOTING
// [KERNEL] Added source code For Sleep Mode Test, Test Mode V8.3 [250-42] 
// LGE_CHANGE [dojip.kim@lge.com] 2010-09-28, ftm boot 
	  //remote_set_ftm_boot(0);
...
/* MOD 0014656: [LG RAPI] OEM RAPI PACKET MISMATCH KERNEL CRASH FIX */
  DIAG_TEST_MODE_F_req_type req_ptr;
  //Suresh: TestScript Mode check
  unsigned char  startStat;
...
//Suresh: START For differentiating UserScript 'n' TestScript modes
//	if(startStatus != FACTORY_RESET_NA)
	{
		startStat = pbuf[FACTORY_RESET_STR_SIZE] - '0';
		printk(KERN_ERR "[JABI] StartStatus [%d - %c]\n", startStatus, startStatus);
		put_device_in_test_script_mode(pbuf);
	}
//Suresh: END
  return pRsp;
}
...
{
  mm_segment_t oldfs;
  struct file *test_mode_fd;
  unsigned int frstmode = buf[FACTORY_RESET_STR_SIZE] - '0';

	printk("[JABI] put_device_in_test_script_mode");
		if((frstmode == 0) || (frstmode == 2)) {
			oldfs = get_fs();
			set_fs(KERNEL_DS);
			
			//	struct file *f = filp_open("/path/to/file", FLAGS, MODE);
			test_mode_fd = filp_open("/persist/factory/testscript", O_CREAT | O_RDWR | O_TRUNC, 0777) ;

			 if (IS_ERR(test_mode_fd) || (test_mode_fd == NULL)) 
			{
				printk(KERN_ERR "[JABI] Can not create testscipt\n");
				printk("[JABI] filp_open returned error %ld\n",PTR_ERR(test_mode_fd));

				set_fs(oldfs);
			}
			printk ("[JABI] flip_open success\n"); 			
		} else
			return;	

 	test_mode_fd->f_pos = 0;
	if(frstmode == 0) {
		int bytes_written = vfs_write(test_mode_fd, buf, strlen(buf), &test_mode_fd->f_pos);
		printk(KERN_ERR "[JABI] In TestScipt mode bytes written [%d]\n", bytes_written);
	} else if (frstmode == 2) {
		int bytes_written = vfs_write(test_mode_fd, buf, strlen(buf), &test_mode_fd->f_pos);
		printk(KERN_ERR "[JABI] UserScript Mode bytes written [%d]\n", bytes_written);
	} else
		printk(KERN_ERR "[JABI] FRST is %s and frstmode [%d] buf[FACTORY_RESET_STR_SIZE] =>[%c]\n", buf, frstmode, buf[FACTORY_RESET_STR_SIZE]);

	printk(KERN_ERR "[JABI] FRST status is [%d] buf[FACTORY_RESET_STR_SIZE] =>[%c]\n", frstmode, buf[FACTORY_RESET_STR_SIZE]);
	set_fs(oldfs);
	
	/* Close the file */
	filp_close(test_mode_fd, NULL);
}
//Suresh: END
...
/* MOD 0014656: [LG RAPI] OEM RAPI PACKET MISMATCH KERNEL CRASH FIX */
      ...
      if(pReq->test_mode_test_scr_mode == TEST_SCRIPT_MODE_CHECK)
      {
        switch(pRsp->test_mode_rsp.test_mode_test_scr_mode)
        {
          case 0:
		  	//Suresh: START TestScript Mode check
		  	put_device_in_test_script_mode("FACT_RESET_2");
            printk(KERN_INFO "%s, mode : %s\n", __func__, "USER SCRIPT");
            break;
          case 1:
		  	//Suresh: START TestScript Mode check
		  	put_device_in_test_script_mode("FACT_RESET_0");
            printk(KERN_INFO "%s, mode : %s\n", __func__, "TEST SCRIPT");
            break;
          default:
		  	//Suresh: START TestScript Mode check
		  	put_device_in_test_script_mode("FACT_RESET_6");
