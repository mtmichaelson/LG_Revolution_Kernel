...
int vt_camera_power_on (void)
{
	...
	/* Input MCLK = 31MHz */
	//msm_camio_clk_rate_set(31000000);	
	msm_camio_clk_rate_set(32000000);	//Lavanya changed from 31MHz to 32MHz to avoid I2c read error thereby causing flip problem
	mdelay(1);
	msm_camio_camif_pad_reg_reset();
	mdelay(1);
