...
/* SBA 203010 in FROYO, merged but not released in GB
 * CRs-Fixed: 272304 269590 - Function/Feature Failure (Complete)
 * RPC Calls/Replies getting stuck in the APPs Router
 * 
 * Problem description
 * APPS RPC router was reading the flow control messages into a
 * unprotected global buffer, before processing it. In a scenario, where
 * both 7K & 9K modems sent the flow control messages almost at the same
 * instant, one of the messages is overwritten by the other in the global
 * buffer. This leads to unblocking the RPC messages towards only one
 * modem and blocking the RPC messages towards other modem forever.
 * 
 * Failure frequency: Occasionally
 * Scenario frequency: Common
 * Change description
 * Implement a Transport specific Router to Router buffer to handle the
 * Router to Router control messages
 * Files affected
 * <android_platform>/kernel/arch/arm/mach-msm/smd_rpcrouter.c
 */
...
/*Begin: RPC packet Null pointer issue.taek.lee@lge.com */
#ifdef CONFIG_LGE_MSM_ONCRPCROUTER_CRASH_BUG_FIX
static int i_msm_rpcrouter_closed = 0;

static int is_msm_rpcrouter_closed(void)
{
	//if(i_msm_rpcrouter_closed == 1)
	//	printk(KERN_ERR "%s, rpcrouter is closed\n");
	return i_msm_rpcrouter_closed;
}
#endif
/*End: taek.lee from MS910*/
...
/* CR 269590 - Stability
 * RPC Reply is blocked waiting on the flow control quota count
 * 
 * Problem description
 * RPC reply is ready to be sent at the APPs side, but APPs RPC Router is
 * waiting for the flow control acknowledgement from both the modem
 * processors. In the meanwhile, both the modem processors sent the flow
 * control acknowledgement. But the APPs reset the flow control quota
 * count for one processor in both the cases, leaving the RPC reply to the
 * other processor blocked forever.
 * 
 * Failure frequency: Occasionally
 * Scenario frequency: Common
 * Change description
 * Double check the remote endpoint/client information before resetting
 * its flow control quota count to avaoid any possible race conditions.
 * Files affected
 * kernel/arch/arm/mach-msm/smd_rpcrouter.c
 */
...
// prevent Null pointer exception or data abort in msm_rpc_write_pkt+0x558/0x7e8
// if rpcrouter has already been closed, then return not supported error.
#ifdef CONFIG_LGE_MSM_ONCRPCROUTER_CRASH_BUG_FIX
	if(is_msm_rpcrouter_closed() == 1)
		return -ENOTSUPP;
#endif
...
/*Begin: taek.lee From MS910*/
#ifdef CONFIG_LGE_MSM_ONCRPCROUTER_CRASH_BUG_FIX
	i_msm_rpcrouter_closed = 1;
	printk(KERN_INFO "%s, set flag %d\n", __func__, i_msm_rpcrouter_closed);	
#endif
/*End: taek.lee From MS910*/
	return 0;
}
