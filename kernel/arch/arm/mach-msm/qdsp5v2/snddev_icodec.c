...
#if 1 //LGE_AUDIO_PATH
{	
	s32 nb_max, nb_min, wb_max, wb_min;
	nb_max = icodec->data->max_voice_rx_vol[VOC_NB_INDEX];
	nb_min = icodec->data->min_voice_rx_vol[VOC_NB_INDEX];
	wb_max = icodec->data->max_voice_rx_vol[VOC_WB_INDEX];
	wb_min = icodec->data->min_voice_rx_vol[VOC_WB_INDEX];
	MM_ERR("[EH]nb_max:%d nb_min:%d wb_max:%d wb_min:%d\n", nb_max, nb_min, wb_max, wb_min);
}	
#endif
...
#if defined (CONFIG_MACH_LGE_BRYCE) //Kiran.kanneganti@lge.com 03 Sep 2010
#define INTERNAL_CODEC 0x00FF
#endif
...
static int snddev_icodec_open_tx(struct snddev_icodec_state *icodec)
{
	...
	for (i = 0; i < icodec->data->pmctl_id_sz; i++) {
		pmic_hsed_set_current_threshold(icodec->data->pmctl_id[i],
		PM_HSED_SC_SWITCH_TYPE,
//		1700);
		icodec->data->micbias_current[i]);
...
static int snddev_icodec_close_rx(struct snddev_icodec_state *icodec)
{
	int err;
	struct snddev_icodec_drv_state *drv = &snddev_icodec_drv;
	MM_INFO("snddev_icodec_close_rx\n");
...
static int snddev_icodec_close_tx(struct snddev_icodec_state *icodec)
{
	...
	MM_INFO("snddev_icodec_close_tx\n");
	wake_lock(&drv->tx_idlelock);
...
/*******************************************************************************
*  Test Mode headset & handset, packet & DSP loopback    ++ kiran.kanneganti@lge.com
*
********************************************************************************/

#if LOOPBACK_SUPPORT

static struct adie_codec_path *testmode_lb_rx_adie;
static struct adie_codec_path *testmode_lb_tx_adie;

/*******************************************************************************
*  Test Mode headset loopback Rx section
*
********************************************************************************/
/*BEGIN: 0011242 ehgrace.kim@lge.com 2010-11-22*/
/*MOD 0011242: Gain Change for handset & headset Loopback*/

/*BEGIN: 0010850 kiran.kanneganti@lge.com 2010-11-15*/
/*MOD 0010850: Gain Change for handset & headset Loopback*/
...
/* MOD 0010969: As it is voice used Mono Config*/

/*BEGIN: 0011021 kiran.kanneganti@lge.com 2010-11-17*/
/*MOD 0011021: Cal data change for call & loopback, HW request*/
/*MOD 0011021: codec RX gain in headset loop back*/

/*BEGIN: 0013991 kiran.kanneganti@lge.com 2011-01-13*/
/*MOD 0013991: Reduced Rx gain to avoid noise. [0x84]*/
/* BEGIN:0014024  kiran.kanneganti@lge.com     2011.01.13*/
/* MOD:0014024 Calibration support for loop back devices  */

/* BEGIN: 0014298 kiran.kanneganti@lge.com 2011-01-18*/
/* MOD 0014298: Reduce Noise in Headset Loopback. 0x84 [0x00 -> 0x0F ]*/
struct adie_codec_action_unit ihs_test_mode_mono_rx_8KHz_osr256_actions[] =
{
...
/* BEGIN:0014024  kiran.kanneganti@lge.com     2011.01.13*/
/* MOD:0014024 Calibration support for loop back devices  */
static struct adie_codec_hwsetting_entry ihs_testmode_lb_rx_settings[] = {
	{
		.freq_plan = 8000,
		.osr = 256,
		.actions = ihs_test_mode_mono_rx_8KHz_osr256_actions,
		.action_sz = ARRAY_SIZE(ihs_test_mode_mono_rx_8KHz_osr256_actions),
	}
};
/* END:0014024  kiran.kanneganti@lge.com     2011.01.13*/
/* END: 0010850 kiran.kanneganti@lge.com 2010-11-15 */
static struct adie_codec_dev_profile testmode_headset_rx_profile = {
	.path_type = ADIE_CODEC_RX,
	.settings = ihs_testmode_lb_rx_settings,
	.setting_sz = ARRAY_SIZE(ihs_testmode_lb_rx_settings),
};

/*******************************************************************************
*  Test Mode headset loopback Tx section
*
********************************************************************************/
/*BEGIN: 0010850 kiran.kanneganti@lge.com 2010-11-15*/
/*MOD 0010850: Gain Change for handset & headset Loopback*/
...
/*BEGIN: 0011021 kiran.kanneganti@lge.com 2010-11-17*/
/*MOD 0011021: Cal data change for call & loopback, HW request*/
/*MOD 0011021: Headset codec tx gain for loopback*/
/* BEGIN:0014024  kiran.kanneganti@lge.com     2011.01.13*/
/* MOD:0014024 Calibration support for loop back devices  */

/* BEGIN: 0014298 kiran.kanneganti@lge.com 2011-01-18*/
/* MOD 0014298: Reduce Noise in Headset Loopback. 0x86 [0x00 -> 0xF1 ]*/
struct adie_codec_action_unit ihs_test_mode_mono_tx_8KHz_osr256_actions[] =
{	
	{ADIE_CODEC_ACTION_STAGE_REACHED,  ADIE_CODEC_FLASH_IMAGE},
	...
	{ADIE_CODEC_ACTION_ENTRY,  ADIE_CODEC_PACK_ENTRY(0x86, 0xff, 0xF1)},
...
/* BEGIN:0014024  kiran.kanneganti@lge.com     2011.01.13*/
/* MOD:0014024 Calibration support for loop back devices  */
static struct adie_codec_hwsetting_entry ihs_testmode_lb_tx_settings[] = {
	{
		.freq_plan = 8000,
		.osr = 256,
		.actions = ihs_test_mode_mono_tx_8KHz_osr256_actions,
		.action_sz = ARRAY_SIZE(ihs_test_mode_mono_tx_8KHz_osr256_actions),
	}
};
/* END:0014024  kiran.kanneganti@lge.com     2011.01.13*/
static struct adie_codec_dev_profile testmode_headset_tx_profile = {
	.path_type = ADIE_CODEC_TX,
	.settings = ihs_testmode_lb_tx_settings,
	.setting_sz = ARRAY_SIZE(ihs_testmode_lb_tx_settings),
};

/*******************************************************************************
*	Function Name :  headset_loopback_for_testmode
*	Args : enable or disable loopback. ( packet or DSP )
*
*	dependencies : If packet loop back check rpc in misc driver. 
********************************************************************************/
#define DSP_LOOPBACK 0x01
/*BEGIN: 0010850 kiran.kanneganti@lge.com 2010-11-15*/
/*MOD 0010850: Gain Change for handset & headset Loopback*/

/* BEGIN: 0010969 kiran.kanneganti@lge.com 2010-11-16 */
/* MOD 0010969: Calibration Data & functions changed for clear voice in loop back test */
/*BEGIN: 0015923 ehgrace.kim@lge.com 2011-02-12*/
/*MOD 0015923: add the routine for exception case for loopback*/
void headset_loopback_for_testmode(u32 loop)
{
	int trc;
	struct msm_afe_config afe_config;
	struct lpa_codec_config lpa_config;
...
*************************************************************************************/
	/* Vote for SMPS mode*/
		trc = pmapp_smps_mode_vote(SMPS_AUDIO_PLAYBACK_ID,
					PMAPP_VREG_S4, PMAPP_SMPS_MODE_VOTE_PFM);
		if (trc != 0)
			MM_ERR("pmapp_smps_mode_vote error %d\n", trc);
	/* enable MI2S RX master block */
	/* enable MI2S RX bit clock */
   ...
   /*enable LPA clock*/
	...
	/* Enable LPA sub system*/
		...
		if (!drv->lpa)
		{
			MM_ERR("failed to get LPA\n");
			goto tm_rx_error_lpa;
		}
		lpa_config.sample_rate = 8000;
		lpa_config.sample_width = 16;
		lpa_config.output_interface = LPA_OUTPUT_INTF_WB_CODEC;
		lpa_config.num_channels = 1; //Baikal ID 0010850
		lpa_cmd_codec_config(drv->lpa, &lpa_config);	
	/* Set audio interconnect reg to LPA */
		audio_interct_codec(AUDIO_INTERCT_LPA);
	/* Set MI2S */
		mi2s_set_codec_output_path(MI2S_CHAN_MONO_PACKED, WT_16_BIT); //Baikal ID 0010850
		MM_INFO("configure ADIE RX path\n");
	/* Configure ADIE */
		trc = adie_codec_open(&testmode_headset_rx_profile, &testmode_lb_rx_adie);
		if (IS_ERR_VALUE(trc))
			goto tm_rx_error_adie;
		adie_codec_setpath(testmode_lb_rx_adie, 8000, 256);
	/* Enable LPA codec*/
		lpa_cmd_enable_codec(drv->lpa, 1);
	/* Enable Afe*/	
		afe_config.sample_rate = 8;
		afe_config.channel_mode = 1; //Baikal ID 0010850
		afe_config.volume = AFE_VOLUME_UNITY;
		MM_INFO("enable afe\n");
		trc = afe_enable(AFE_HW_PATH_CODEC_RX, &afe_config);
		if (IS_ERR_VALUE(trc))
		{
			MM_ERR("fail to enable afe rx\n");
			goto tm_rx_error_afe;
		}
	/*start adie streaming*/	
		adie_codec_proceed_stage(testmode_lb_rx_adie,
		ADIE_CODEC_DIGITAL_ANALOG_READY);
	/*Set amp for Headset Path*/
		set_amp_gain(6);  //for lge_snddev_hs_phone_amp_on
	/*Enable SideTone */
		adie_codec_enable_sidetone(testmode_lb_rx_adie, 0x01);
/************************************************************************************
*  Tx Path Settings
*************************************************************************************/
	/* Vote for PWM mode*/
	//Baikal ID 0010850
	trc = pmapp_smps_mode_vote(SMPS_AUDIO_RECORD_ID,
			PMAPP_VREG_S4, PMAPP_SMPS_MODE_VOTE_PWM);
	if (trc != 0)
		MM_ERR("pmapp_smps_mode_vote error %d\n", trc);
	/* enable MI2S TX master block */
	/* enable MI2S TX bit clock */
		trc = clk_set_rate(drv->tx_mclk,SNDDEV_ICODEC_CLK_RATE(8000));
		if (IS_ERR_VALUE(trc))
			goto tm_tx_error_invalid_freq;
		clk_enable(drv->tx_mclk);
		clk_enable(drv->tx_sclk);
	/* Set MI2S */
		mi2s_set_codec_input_path(MI2S_CHAN_MONO_RAW, WT_16_BIT);
		MM_INFO("configure ADIE TX path\n");
	/* Configure ADIE */
		trc = adie_codec_open(&testmode_headset_tx_profile, &testmode_lb_tx_adie);
		if (IS_ERR_VALUE(trc))
			goto tm_tx_error_adie;
		adie_codec_setpath(testmode_lb_tx_adie, 8000, 256);
		adie_codec_proceed_stage(testmode_lb_tx_adie,
		ADIE_CODEC_DIGITAL_ANALOG_READY);
	/* Start AFE */
		afe_config.sample_rate = 8;
		afe_config.channel_mode = 1;
...
*************************************************************************************/	
	/* Set the volume level to non unity, to avoid
			   loopback effect */
		afe_device_volume_ctrl(AFE_HW_PATH_CODEC_RX, 0x0500);
		if( DSP_LOOPBACK == (loop >> 1))
		{
			MM_INFO("configure DSP LoopBack path\n");
			afe_config_loopback(0xFFFF);
		}	
	}
	else
	{
	...
	/* Remove the vote for SMPS mode*/
		...
		if (trc != 0)
			MM_ERR("pmapp_smps_mode_vote error %d\n", trc);		
    /*Disable SideTone */
		adie_codec_enable_sidetone(testmode_lb_rx_adie, 0x0);	
	/* Disable rx ADIE */
		adie_codec_proceed_stage(testmode_lb_rx_adie,
		ADIE_CODEC_DIGITAL_OFF);
		adie_codec_close(testmode_lb_rx_adie);
	/* Disable tx ADIE */
		adie_codec_proceed_stage(testmode_lb_tx_adie,
		ADIE_CODEC_DIGITAL_OFF);
		adie_codec_close(testmode_lb_tx_adie);
/************************************************************************************
*   Stop LoopBack 
*************************************************************************************/
		if( loop )
		{
			MM_INFO("DISABLE DSP LOOPBACK \n");
			afe_config_loopback(0x0);
		}	
	/* Disable rx AFE */
		afe_disable(AFE_HW_PATH_CODEC_RX);
	/* Disable tx AFE */	
...
tm_tx_error_afe:
	adie_codec_close(testmode_lb_tx_adie);
//	testmode_lb_tx_adie = NULL;
tm_tx_error_adie:
	clk_disable(drv->tx_sclk);
	clk_disable(drv->tx_mclk);
tm_tx_error_invalid_freq:
	set_amp_gain(5);
	adie_codec_enable_sidetone(testmode_lb_rx_adie, 0x0);	
	MM_ERR("tx encounter error\n");

tm_rx_error_afe:
	adie_codec_close(testmode_lb_rx_adie);
//	testmode_lb_rx_adie = NULL;
tm_rx_error_adie:
	lpa_put(drv->lpa);
tm_rx_error_lpa:
	clk_disable(drv->lpa_p_clk);
	...
	clk_disable(drv->rx_sclk);
	clk_disable(drv->rx_mclk);
tm_rx_error_invalid_freq:
	MM_ERR("rx encounter error\n");

	return;
}
/*END: 0015923 ehgrace.kim@lge.com 2011-02-12*/
/* END: 0010969 kiran.kanneganti@lge.com 2010-11-16 */ 

/* END: 0010850 kiran.kanneganti@lge.com 2010-11-15 */
EXPORT_SYMBOL(headset_loopback_for_testmode);
#if 1 //For QCOM Dual MIC loopback
/*******************************************************************************
*  Test Mode handset loopback Rx section
*
********************************************************************************/
/*BEGIN: 0010850 kiran.kanneganti@lge.com 2010-11-15*/
/*MOD 0010850: Gain Change for handset & headset Loopback*/
...
/*BEGIN: 0011021 kiran.kanneganti@lge.com 2010-11-17*/
/*MOD 0011021: Cal data change for call & loopback, HW request*/
/*MOD 0011021: Handset loopback codec rx gain*/

/*BEGIN: 0012726 kiran.kanneganti@lge.com 2010-12-20*/
/*Mod 0012726:  Flash marimba codec image before start.*/
/* BEGIN:0014024  kiran.kanneganti@lge.com     2011.01.13*/
/* MOD:0014024 Calibration support for loop back devices  */
struct adie_codec_action_unit testmode_handset_rx_8KHz_osr256_actions[] =
{
	{ADIE_CODEC_ACTION_STAGE_REACHED,  ADIE_CODEC_FLASH_IMAGE},
	{ ADIE_CODEC_ACTION_DELAY_WAIT, 0xbb8},
	{ADIE_CODEC_ACTION_ENTRY, ADIE_CODEC_PACK_ENTRY(0x80,  0x02,  0x02)},
...
/* END:0014024  kiran.kanneganti@lge.com     2011.01.13*/
/*END: 0012726 kiran.kanneganti@lge.com 2010-12-20*/
/* END: 0011021 kiran.kanneganti@lge.com 2010-11-17 */
/* END: 0010850 kiran.kanneganti@lge.com 2010-11-15 */
static struct adie_codec_hwsetting_entry handset_testmode_lb_rx_settings[] = {
	{
		.freq_plan = 8000,
		.osr = 256,
		.actions = testmode_handset_rx_8KHz_osr256_actions,
		.action_sz = ARRAY_SIZE(testmode_handset_rx_8KHz_osr256_actions),
	}
};
static struct adie_codec_dev_profile testmode_handset_rx_profile = {
	.path_type = ADIE_CODEC_RX,
	.settings = handset_testmode_lb_rx_settings,
	.setting_sz = ARRAY_SIZE(handset_testmode_lb_rx_settings),
};
/*******************************************************************************
*  Test Mode handset loopback Tx section
*
********************************************************************************/
/*BEGIN: 0010850 kiran.kanneganti@lge.com 2010-11-15*/
/*MOD 0010850: Gain Change for handset & headset Loopback*/
...
/*BEGIN: 0011021 kiran.kanneganti@lge.com 2010-11-17*/
/*MOD 0011021: Cal data change for call & loopback, HW request*/
/*MOD 0011021: handset loopback codec tx gain*/
/*BEGIN: 0011453 kiran.kanneganti@lge.com 2010-11-26*/
/*ADD 0011453: Front Mic & Back Mic Separate LoopBack Support*/
/*MOD 0011453: Changed to Front Mic active*/

/*BEGIN: 0012726 kiran.kanneganti@lge.com 2010-12-20*/
/*Mod 0012726:  Flash marimba codec image before start.*/
/* BEGIN:0014024  kiran.kanneganti@lge.com     2011.01.13*/
/* MOD:0014024 Calibration support for loop back devices  */
struct adie_codec_action_unit testmode_handset_tx_8KHz_osr256_actions[] =
{
	{ADIE_CODEC_ACTION_STAGE_REACHED,  ADIE_CODEC_FLASH_IMAGE},
	{ADIE_CODEC_ACTION_ENTRY, ADIE_CODEC_PACK_ENTRY(0x80, 0x01, 0x01)},
	{ADIE_CODEC_ACTION_ENTRY, ADIE_CODEC_PACK_ENTRY(0x80, 0x01, 0x00)},
...
/* END: 0011021 kiran.kanneganti@lge.com 2010-11-17 */
/* END: 0010850 kiran.kanneganti@lge.com 2010-11-15 */
static struct adie_codec_hwsetting_entry handset_testmode_lb_tx_settings[] = {
	{
		.freq_plan = 8000,
		.osr = 256,
		.actions = testmode_handset_tx_8KHz_osr256_actions,
		.action_sz = ARRAY_SIZE(testmode_handset_tx_8KHz_osr256_actions),
	}
};
static struct adie_codec_dev_profile testmode_handset_tx_profile = {
	.path_type = ADIE_CODEC_TX,
	.settings = handset_testmode_lb_tx_settings,
	.setting_sz = ARRAY_SIZE(handset_testmode_lb_tx_settings),
};
/*BEGIN: 0011453 kiran.kanneganti@lge.com 2010-11-26*/
/*ADD 0011453: Front Mic & Back Mic Separate LoopBack Support*/
/*Back Mic tx Settings ++*/

/*BEGIN: 0012726 kiran.kanneganti@lge.com 2010-12-20*/
/*Mod 0012726:  Flash marimba codec image before start.*/
/*Mod 0012726:  Mic 2 loop back Hauling effect issue fix.*/

/*BEGIN: 0013956 kiran.kanneganti@lge.com 2011-01-12*/
/*Mod 0013956: Mic2 Loop Back Mic amp gain change to avoid hauling. (0x0D) */
/* BEGIN:0014024  kiran.kanneganti@lge.com     2011.01.13*/
/* MOD:0014024 Calibration support for loop back devices  */

/*BEGIN: 0014140 kiran.kanneganti@lge.com 2011-01-15*/
/*MOD 0014140: apply calibration data from HW team [0x86 00-> 0C]*/

/*BEGIN: 0014838 kiran.kanneganti@lge.com 2011-01-26*/
/*MOD 0014838: Reduce Mic2 loopback tx gain Factory request [0x86 0C-> 06]*/
/*BEGIN: 0016258 ehgrace.kim@lge.com 2011-02-17*/
/*MOD 0014838: Reduce Mic2 loopback tx gain Factory request [0x86 06-> 0A]*/
struct adie_codec_action_unit testmode_handset_back_mic_tx_8KHz_osr256_actions[] =
{
	{ADIE_CODEC_ACTION_STAGE_REACHED,  ADIE_CODEC_FLASH_IMAGE},
	{ADIE_CODEC_ACTION_ENTRY, ADIE_CODEC_PACK_ENTRY(0x80, 0x01, 0x01)},
	{ADIE_CODEC_ACTION_ENTRY, ADIE_CODEC_PACK_ENTRY(0x80, 0x01, 0x00)},
...
/*END: 0014838 kiran.kanneganti@lge.com 2011-01-26 */
/*END: 0014140 kiran.kanneganti@lge.com 2011-01-15 */
/* END:0014024  kiran.kanneganti@lge.com     2011.01.13*/
/*END: 0013956 kiran.kanneganti@lge.com 2011-01-12*/

/*END: 0012726 kiran.kanneganti@lge.com 2010-12-20*/

static struct adie_codec_hwsetting_entry handset_testmode_back_mic_lb_tx_settings[] = {
	{
		.freq_plan = 8000,
		.osr = 256,
		.actions = testmode_handset_back_mic_tx_8KHz_osr256_actions,
		.action_sz = ARRAY_SIZE(testmode_handset_back_mic_tx_8KHz_osr256_actions),
	}
};
static struct adie_codec_dev_profile testmode_handset_back_mic_tx_profile = {
	.path_type = ADIE_CODEC_TX,
	.settings = handset_testmode_back_mic_lb_tx_settings,
	.setting_sz = ARRAY_SIZE(handset_testmode_back_mic_lb_tx_settings),
};
/*Back Mic tx Settings -- */
#define BACK_MIC_ON 0x04
/* END: 0011453 kiran.kanneganti@lge.com 2010-11-26 */
/*BEGIN: 0015923 ehgrace.kim@lge.com 2011-02-12*/
/*MOD 0015923: add the routine for exception case for loopback*/
void QTR_handset_loopback_for_testmode(u32 loop)
{
	int trc;
	struct msm_afe_config afe_config;
	struct lpa_codec_config lpa_config;
...
*************************************************************************************/
   ...
   /*enable LPA clock*/
	...
	/* Set MI2S */
		...
		MM_INFO("configure ADIE RX path\n");
	/* Configure ADIE */
		trc = adie_codec_open(&testmode_handset_rx_profile, &testmode_lb_rx_adie);
		if (IS_ERR_VALUE(trc))
			goto tm_rx_error_adie;
		adie_codec_setpath(testmode_lb_rx_adie, 8000, 256);
	/* Enable LPA codec*/
...
/*ADD 0011453: Front Mic & Back Mic Separate LoopBack Support*/		
	...
	/* Configure ADIE */
		if( loop & BACK_MIC_ON )
		{
			MM_INFO("configure Back Mic ADIE TX path\n");
			trc = adie_codec_open(&testmode_handset_back_mic_tx_profile, &testmode_lb_tx_adie);
			if (IS_ERR_VALUE(trc))
				goto tm_tx_error_adie;
		}
		else
		{
			MM_INFO("configure Front Mic ADIE TX path\n");
			trc = adie_codec_open(&testmode_handset_tx_profile, &testmode_lb_tx_adie);
			if (IS_ERR_VALUE(trc))
				goto tm_tx_error_adie;
		}
		adie_codec_setpath(testmode_lb_tx_adie, 8000, 256);
...
/*END: 0015923 ehgrace.kim@lge.com 2011-02-12*/
EXPORT_SYMBOL(QTR_handset_loopback_for_testmode);
#endif
