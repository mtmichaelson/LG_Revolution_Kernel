...
#define IPV6_NULL_IID {0,0,0,0,0,0,0,0}

static const __u8 ipv6_null_iid[] = IPV6_NULL_IID;
...
//TELECA_NETMGR_DHCP START
	...
	if (memcmp(idev->cnf.iid,ipv6_null_iid,sizeof(ipv6_null_iid)) == 0) {
		switch (dev->type) {
		case ARPHRD_ETHER:
		case ARPHRD_FDDI:
		...
		memcpy(eui,idev->cnf.iid,sizeof(idev->cnf.iid));
	        return 0;
	}
//TELECA_NETMGR_DHCP END
	return -1;
